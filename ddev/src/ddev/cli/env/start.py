    ...
    *,
    ...
    with temp_directory() as temp_dir:
        ...
        if not result_file.is_file():  # no cov
            app.abort(f'No E2E result file found at {result_file}')
    ...
    if not agent_build:
        agent_build = app.config.agent.config.get(agent_type) or ''
    ...
    sep = app.style_warning('->')
    app.output()
    app.output(app.style_success('Config file'), sep, f'[link={env_data.config_file}]{env_data.config_file}[/]')
    app.output(app.style_success('Edit config'), sep, app.style_info(f'ddev env edit {intg_name} {environment}'))
    app.output(app.style_success('Reload config'), sep, app.style_info(f'ddev env edit {intg_name} {environment}'))
    app.output(app.style_success('Trigger run'), sep, app.style_info(f'ddev env check {intg_name} {environment}'))
    app.output(app.style_success('Stop env'), sep, app.style_info(f'ddev env stop {intg_name} {environment}'))
...
def _get_agent_env_vars(org_config, metadata, extra_env_vars, dogstatsd):
    ...
    env_vars = dict(ev.split('=', maxsplit=1) for ev in extra_env_vars)
    ...
    if any(ev.startswith(E2EEnvVars.LOGS_DIR_PREFIX) for ev in env_vars):
